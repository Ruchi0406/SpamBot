...
def load_plugins(plugin_name):
    path = Path(f"DevXSPam/modules/{plugin_name}.py")
    spec = importlib.util.spec_from_file_location(f"AltBots.modules.{plugin_name}", path)
    load = importlib.util.module_from_spec(spec)
    load.logger = logging.getLogger(plugin_name)
    spec.loader.exec_module(load)
    sys.modules["DevXSPam.modules." + plugin_name] = load
    print("DevXSpam has Imported " + plugin_name)


files = glob.glob("DevXSPam/modules/*.py")
for name in files:
    with open(name) as a:
        patt = Path(a.name)
...
print("\n𝐃𝐞𝐯𝐗𝐒𝐩𝐚𝐦 𝐃𝐞𝐩𝐥𝐨𝐲𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 🎄\nMy Master ---> @DamnDevansh")
