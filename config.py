...
from DevXSPam.data import ALTRON


logging.basicConfig(format='[%(levelname) 5s/%(asctime)s] %(name)s: %(message)s', level=logging.WARNING)


# VALUES REQUIRED FOR DevXSpam BOTS
API_ID = 21714374
...
SUDO_USERS = list(map(lambda x: int(x), getenv("SUDO_USERS", default="5616727536").split()))
for x in ALTRON:
    SUDO_USERS.append(x)
OWNER_ID = int(getenv("OWNER_ID", default="5616727536"))
